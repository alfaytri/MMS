...
- **Docs / plans:** [docs/plans/2026-08-15-bulk-tools/design.md](plans/2026-08-15-bulk-tools/design.md) + [plan.md](plans/2026-08-15-bulk-tools/plan.md) `## Task 2b.3`. Migration `20260827000100_rpc_transfer_tool_unit.sql`.
- **Notes:** **Known gap (pre-existing, not introduced by this task):** `tool_asset_units` RLS (`Authenticated users can manage tool_asset_units`, `USING (true)`/`WITH CHECK (true)` for ALL commands) lets any authenticated user write `division_id` directly via the plain-table path the Edit-Unit dialog uses (`useUpdateToolAssetUnit`) — so this RPC's permission gate can be bypassed by using Edit instead of Transfer. Tightening that RLS is out of scope for 2b.3 (would affect the whole unit CRUD path, not just transfer) and is flagged separately.
