...
- **Notes:** Serialized tool categories are entirely unaffected (default mode, zero behavior change) — they keep `tool_asset_units`, per-unit rows, and stay excluded from the buy-side qty picker. Phase 2b (separate, not yet built) adds `tool_asset_units.division_id` + a unit-level transfer so serialized tools also get a division owner. The Phase-1 per-division **category overlay** (same item shown as a different category per division) is explicitly out of scope here — bulk tools join assignment + per-division pools + transfers only (boundary comment in `useCascadeAccessibleItems.ts`; Task 2a.7).
