...
- **COGS reversal (Phase 3a — full-line, operator-confirmed; migrations `20260831000700`–`001100` + `001300`):** every processed disposition now reverses the original sale's COGS **and** revenue via the shared `_reverse_sale_cogs_for_return(return, variant, qty)` — a negative `cogs_entries('sale_return')`, mirroring good-restock (rpc_process_return_restock), sale-sourced returns only. Wiring: `restock_as_damaged` reverses inside `_record_inventory_disposition` (cost moves sold→damaged asset); `write_off` reverses + emits the `inventory_damaged_movements('damaged_write_off')` the P&L Scrap line reads (previously only a P&L-invisible `sale_return_damaged` stock movement) — and fixes a drift bug where that movement omitted the now-NOT-NULL `sub_container_id`, breaking every write-off — then a **second gap caught by the 2026-08-30 operator smoke** (migration `20260831001300`): that sub-container is placed in the return's division, but real sale returns are created with a NULL `division_id`, so `_find_or_create_sub_container` on a real (non-virtual) warehouse still rejected every write-off with "division_id required"; the fix resolves the division via `coalesce(so_po_returns.division_id, sale_orders.division_id)` (the same fallback `rpc_create_partial_replacement` already shipped) and raises a clear hint if still unresolvable — no data backfill needed; `send_for_repair` reverses in [[Send Damaged for Repair]] (once, when the unit leaves "sold"; return-from-repair does NOT re-reverse). The **replacement** ([[Create Partial Replacement (customer + inventory atomic)]]) now books the free swap's + gift's cost via `deduct_fifo_layers` + `cogs_entries(source_type='sale_replacement')` + a `sale_delivery` movement (previously shipped with NO COGS), and `rpc_report_pnl` counts that cost but forces its **revenue to 0** (free like-for-like swap). Good-restock is unchanged (already correct).
...
- **Docs / plans (Phase 3a):** [docs/plans/2026-08-30-phase3a-sales-return-cogs-fix.md](plans/2026-08-30-phase3a-sales-return-cogs-fix.md)
...
- **Status:** Active
- **Trigger surface(s):** Contract detail page (`/contracts/detail/[contractId]`) → **Team Scheduling** section — an order-scheduler-style UI: a **service list** (each pill with an **hours** selector) is dragged onto a rich **half-hour drag-drop calendar** (division-grouped teams, Fit/Scroll, now-line, work-window shading) at a team + time; a **placed** list removes a visit. The calendar shows the team's **real availability** (orders / other contracts / site-visits) for the day. The placed visit also appears on the global `/calendar` (Team × time), coloured as a Contract Visit.
...
- **Downstream side-effects:** The `calendar_visits` VIEW's contract branch now emits real `start_time`/`end_time` (was hardcoded NULL), so a placed visit shows on `/calendar` at its time. Invalidates `contracts.schedule`, `contracts.detail`, and `calendar.visitsAll`. Per-service default block length comes from `services.duration` (**minutes** → rounded up to whole-hour blocks; fallback 2h), overridable per visit via the from→to editor.
- **Dialog / component:** [`ServiceScheduleSection`](src/components/contracts/ServiceScheduleSection.tsx) — **reuses the order calendar primitives** ([`CalendarBlocks`](src/components/orders/CalendarBlocks.tsx): `DroppableCell`/`VisitBlock`/`DivisionHeaderRow` + [`lib/calendar/time`](src/lib/calendar/time.ts) 48-slot half-hour axis, fit-to-window, track-stacking). Placed contract visits render as the SAME `VisitBlock` (they are `calendar_visits` rows), so the contract scheduler, the order scheduler, and `/calendar` all look identical.
- **Guards / preconditions:** contract live (non-quotation) with generated visits; only teams whose `division.slug` is in the contract's `divisions` are shown. **Double-booking guard:** `contract_visit_no_team_overlap` EXCLUDE constraint (btree_gist) blocks two non-completed visits for the same team from overlapping in time on the same day (`chk_contract_visit_time_order` enforces end > start); the hook maps `23P01`/`23514` to a friendly toast. The old fake "Push to Calendar" toast is retired — a visit is live on the calendar the moment it has a team + time.
...
- **Docs / plans:** migrations `supabase/migrations/20261074000000_contract_visit_scheduling_times.sql` + `20261075000000_contract_visit_no_overlap.sql`.
- **Notes:** Contract visits carry a real from→to (`start_time`/`end_time`) mirroring how orders carry `time_slot`+`duration`. The per-service default duration is the existing service-editor **"Duration (minutes)"** field (`services.duration`) — set it on the master service and it flows to scheduling via `contract_services.service_id`.
...
- **RPC(s):** `rpc_set_item_divisions(p_item_id uuid, p_division_ids uuid[])` (SECURITY DEFINER replace-set, gated on `inventory.catalog.manage`, revoked from public); `rpc_item_divisions_by_stock(p_type text)` (STABLE DEFINER — item→divisions = explicit assignment ∪ where-stock-currently-sits ∪ **divisions inherited from the item's category chain** via `inventory_category_divisions`, recursive ancestor walk added 2026-08-24 — see [[Assign Divisions to Category (cascade)]]).
...
- **Related flows:** [[Create Purchase Order (draft or RFQ)]] (buy-side consumer of the assignment membership), [[Set Item Attribute Values]] (same `ItemEditDialog`), [[Bulk Tools — per-category tracking mode]] (bulk tool items join this flow via the same dialog), [[Assign Divisions to Category (cascade)]] (category-level layer + live inheritance; the item dialog now shows inherited divisions checked+locked, and inbound stock auto-sticks an explicit row).
...
### Assign Divisions to Category (cascade)

- **Module:** Master Data · Inventory (incl. Tools & Assets)
- **Status:** Active (2026-08-24, `deploy/warehouse-shipping`, subagent-driven build — **staging-applied; new-prod pending the Task 12 ship gate**)
- **Trigger surface(s):** `/master-data/inventory` and the Tools & Assets tab → any category → Edit → [`CategoryEditDialog`](src/components/services/inventory/CategoryEditDialog.tsx) → **"Assigned divisions"** checklist (own = editable; inherited-from-parent = checked+locked). Tools categories also get an opt-in **"Move all units to home division"** button.
- **Primary hook(s):** [`useCategoryDivisions` + `useSetCategoryDivisions` + `useCascadeCategoryUnitsDivision`](src/hooks/useCategoryDivisions.ts); the item dialog reads [`useItemEffectiveDivisions`](src/hooks/useItemDivisions.ts) to render inherited-locked rows.
- **RPC(s):** `rpc_set_category_divisions(p_category_id uuid, p_division_ids uuid[])` (SECDEF replace-set, gated `_user_can_write_catalog`); `rpc_category_divisions(p_category_id uuid)` → `{own, inherited}` (own + **strict-ancestor** inherited); `rpc_item_effective_divisions(p_item_id uuid)` → `{explicit, inherited}` (explicit + the item's **whole** category chain incl. self); `rpc_cascade_category_units_division(p_category_id uuid, p_division_id uuid)` → `{moved, skipped}` (opt-in tool-unit re-home, gated `inventory.catalog.manage`, mirrors [[Transfer Serialized Tool Unit]] semantics). All revoked from public/anon, granted authenticated.
- **Ledger writes:** `public.inventory_category_divisions` (category_id, division_id, created_at, created_by; PK `(category_id, division_id)`). RLS `icd_select` (SELECT `TO authenticated`); **no direct write policy** — writes only via the SECDEF RPCs (stricter than the sibling `inventory_item_divisions`). The tool-unit re-home writes `tool_asset_units.division_id` + releases open `tool_unit_assignments`.
- **Downstream side-effects:** `useSetCategoryDivisions` invalidates `['category-divisions']`, `['item-divisions-by-stock']`, `['item-effective-divisions']`, `['cascade-accessible','assignment']`. **Live inheritance:** assigning a division to a category instantly widens the effective divisions of every sub-category + item under it (read-time, no materialization) through [[Assign Item to Divisions]]'s `rpc_item_divisions_by_stock`.
- **Dialog / component:** [`CategoryEditDialog`](src/components/services/inventory/CategoryEditDialog.tsx) (shared by inventory + tools).
- **Guards / preconditions:** category-division writes require `_user_can_write_catalog` (create/edit/manage); unit re-home requires `inventory.catalog.manage`. **Additive-only:** inherited divisions render locked and can never be dropped at a lower level.
- **Related flows:** [[Assign Item to Divisions]] (item-level explicit layer + the read RPC this extends), [[Transfer Serialized Tool Unit]] (unit re-home reuses its semantics), [[Bulk Tools — per-category tracking mode]] (same dialog + cascade-RPC pattern).
- **Docs / plans:** [docs/specs/2026-08-24-category-division-cascade-design.md](specs/2026-08-24-category-division-cascade-design.md) + [docs/plans/2026-08-24-category-division-cascade.md](plans/2026-08-24-category-division-cascade.md). Migrations `20261005000000`–`000400` (staging; new-prod at ship), mirrored to `supabase/migrations-staging/`.
- **Notes:** Also adds an **auto-stick trigger** `trg_autostick_item_division` (AFTER INSERT on `fifo_cost_layers`): any inbound stock landing in a division's container now writes a permanent `inventory_item_divisions` row (additive, idempotent via `ON CONFLICT DO NOTHING`, never removes), making the formerly-transient stock-derived membership stick. This is what turns [[Assign Item to Divisions]]'s read into a 3-way union. Tool-unit re-home is **opt-in** (a button, not automatic on save), so a unit transferred elsewhere is never silently snapped back.

### Bulk Tools — per-category tracking mode

- **Module:** Master Data · Inventory / Warehouse
- **Status:** Active — Phase 2a (2026-08-15), branch `feature/bulk-tools`, staging only. **Selling extension + visual mode-language added 2026-08-30** (Selling Bulk Tools plan — bulk tools are now sellable via the sale order; staging, not committed/pushed).
...
- **Ledger writes:** `inventory_categories.tool_tracking_mode` (enum `tool_tracking_mode`; **default flipped `'serialized'`→`'bulk'`** in migration `20260831001200` so new tool categories are sellable-by-default; original column migration `20260826000000`). Bulk tool items otherwise write the same ledgers as any qty item — `inventory_items` + `inventory_item_brand_variants` + `fifo_cost_layers` + `inventory_stock_movements` + `inventory_item_divisions` (via [[Assign Item to Divisions]]) — never `tool_asset_units`.
...
- **Guards / preconditions:** `tool_tracking_mode` is meaningful only when `inventory_categories.type='tools'` (ignored otherwise). Switching mode is blocked **server-side** (not just client-disabled) while the category has stock/units. **Bulk tools ARE sellable to customers** (operator decision 2026-08-30): `SoLineItemsEditor.ALL_TYPES` now includes `'tools'`, so a bulk tool sells through the normal SO → delivery → COGS('sale') path like a consumable (the sale/delivery RPCs `create_sale_order`/`create_and_confirm_delivery`/`complete_delivery_inventory` carry no type gate — verified against the live DB). **Serialized tools stay out of sales**, enforced by the PICKER not the SO editor: `CascadeInventorySelector` runs the sell side (`divisionFilterRequiresStock` default true) → `useCascadeAccessibleItems` filters `type='tools'`→`tool_tracking_mode='bulk'` owned-stock only, so a serialized unit can never surface in an SO. Warranty rides the type-agnostic [[Create Warranty Records at Delivery]] hook.
...
- **Migrations:** `20260826000000_inventory_categories_tool_tracking_mode.sql` (enum + column), `20260826000100_receival_tool_units_serialized_only.sql` (mode-aware receival trigger, rebased on the live function body), `20260826000200_guard_tool_tracking_mode_switch.sql` (populated-guard trigger); `20260831001200_default_tool_tracking_mode_bulk.sql` (default→`'bulk'`, Selling Bulk Tools Task 1). All staging-only (`mwvblpgbgxipvrevkeff`), mirrored to `supabase/migrations-staging/`.
- **Notes:** Serialized tool categories are entirely unaffected (default mode, zero behavior change) — they keep `tool_asset_units`, per-unit rows, and stay excluded from the buy-side qty picker. **Phase 2b (in progress, same branch):** Task 2b.1 added `tool_asset_units.division_id` (nullable, no backfill), Task 2b.2 added a Division select to the unit edit dialog, and Task 2b.3 added the dedicated unit-level transfer — see [[Transfer Serialized Tool Unit]]. The Phase-1 per-division **category overlay** (same item shown as a different category per division) is explicitly out of scope here — bulk tools join assignment + per-division pools + transfers only (boundary comment in `useCascadeAccessibleItems.ts`; Task 2a.7). **Visual mode-language (Selling Bulk Tools Task 4, 2026-08-30):** shared `ToolModeBadge` + `ToolModeDot` in [`ToolBadges.tsx`](src/components/warehouse/tools-assets/ToolBadges.tsx) — **Bulk = teal · Layers · "Sellable"**, **Serialized = violet · Hash · "Custody"** — tell the two apart at a glance across `ToolCategoryRow` (category chip), the item rows (a coloured dot on `BulkToolItemRow` + the serialized `ToolItemRow`), and a mode legend strip in `ToolsAssetsView`. Violet/teal are semantic MODE accents only; the app primary stays orange.
...
### Create Warranty Records at Consumption

- **Module:** Operations / Consumption / Warranty
- **Status:** Active
- **Trigger surface(s):** Server-side. Fired inline by [`rpc_post_consumption`](supabase/migrations/20260831000300_rpc_post_consumption_warranty_hook.sql) when a **custody** consumption is posted from [`NewConsumptionDialog`](src/components/consumption/NewConsumptionDialog.tsx). No operator-facing trigger of its own. Read side: the [`/consumption/warranties`](src/app/(dashboard)/consumption/warranties/page.tsx) page.
- **Primary hook(s):** No dedicated mutation hook — DB-owned. Read-side: [`useWarrantyRecords`](src/hooks/useWarrantyRecords.ts) with `source: 'consumption'`, [`useWarrantyClaims`](src/hooks/useWarrantyClaims.ts) with `warrantyType: 'consumption'`.
- **RPC(s):** [`public.rpc_post_consumption`](supabase/migrations/20260831000300_rpc_post_consumption_warranty_hook.sql) calls [`public.create_warranty_records_for_consumption(p_consumption_id)`](supabase/migrations/20260831000200_create_warranty_records_for_consumption.sql) inline after the average-cost recalc, before returning. Resolver: [`public.get_effective_warranty_policy(p_item_id)`](supabase/migrations/20260815003300_get_effective_warranty_policy.sql). Numbering: `public.next_warranty_number('consumption', division_id)`. Confirm the live body via `pg_get_functiondef` before editing again.
- **Ledger writes:** `warranty_records` — one row per **custody** `consumption_lines` row whose item resolves an effective policy (`duration_months > 0`), with `source_type='consumption'`, `consumption_id` + `consumption_line_id` provenance (sale linkage columns NULL — enforced by the `warranty_records_source_xor` CHECK added in [`20260831000100`](supabase/migrations/20260831000100_warranty_records_consumption_source.sql)), snapshotted policy fields, origin snapshot, `warranty_number` from `next_warranty_number('consumption', division_id)`, and **`start_date` = the consumption date** (`end_date = start + duration_months`). `division_id` denormed for RLS.
- **Downstream side-effects:** None outside `warranty_records`. `ON CONFLICT (consumption_line_id) DO NOTHING` (unique index) gives free idempotency on retry. Claims filed later go through the shared warranty-claim RPCs, which pick the permission key by the record's `source_type` ([`20260831000400`](supabase/migrations/20260831000400_warranty_claim_rpcs_source_aware_perm.sql)).
- **Dialog / component:** N/A for creation (server-side). Records + claims render on [`ConsumptionWarrantiesPage`](src/app/(dashboard)/consumption/warranties/page.tsx); claims reuse [`FileWarrantyClaimDialog`](src/components/sales/FileWarrantyClaimDialog.tsx) (scoped `source="consumption"`) + [`WarrantyClaimDetailDialog`](src/components/sales/WarrantyClaimDetailDialog.tsx).
- **Guards / preconditions:** **Custody-only** — internal consumption creates none (checked inside the function: `consumer_type='custody'` AND `division_id` not null). Lines skip silently when (a) no `brand_variant_id`, (b) `qty <= 0`, (c) resolved policy is NULL, or (d) `duration_months = 0`. Page view requires `consumption.warranties.view`; claim management requires `consumption.warranty_claims.manage`.
- **Related flows:** [[Create Warranty Records at Delivery]]
- **Docs / plans:** [docs/plans/2026-08-30-consumption-phase2-warranty.md](plans/2026-08-30-consumption-phase2-warranty.md), [docs/plans/2026-08-29-consumption-sales-returns-warranty-design.md](plans/2026-08-29-consumption-sales-returns-warranty-design.md)
- **Notes:** Consumption has no delivery/invoice split, so `start_date` is always the consumption date regardless of the policy's `starts_from` (the policy value is still snapshotted). Consumption warranties carry **no customer** — the invoice/order/project ref lives in the consumption's `notes` (Phase 1 made notes mandatory for custody), so the page shows Division where Sales shows Customer. Phase 4 (warranty claim → consumption return) is **built** — see [[Warranty Claim (sale + consumption)]] and [[Consumption Return]].
...
### Warranty Claim (sale + consumption)

- **Module:** Sales / Warranty · Operations / Consumption
- **Status:** Active
- **Trigger surface(s):** [`/sales/warranties`](src/app/(dashboard)/sales/warranties/page.tsx) (sale warranties) and [`/consumption/warranties`](src/app/(dashboard)/consumption/warranties/page.tsx) (consumption warranties, Phase 4) → **Claims** tab (list + **File a claim**) and the **File claim** button on a warranty record's detail dialog. Actions from the shared [`WarrantyClaimDetailDialog`](src/components/sales/WarrantyClaimDetailDialog.tsx): Assess (Cover / Reject), Start resolution, Void. File entry: [`FileWarrantyClaimDialog`](src/components/sales/FileWarrantyClaimDialog.tsx).
...
- **RPC(s):** [`public.rpc_file_warranty_claim(p_warranty_record_id, p_issue)`](supabase/migrations/20261002000400_warranty_claim_rpcs.sql), [`public.rpc_assess_warranty_claim(p_claim_id, p_decision, p_reason)`](supabase/migrations/20261002000400_warranty_claim_rpcs.sql), [`public.rpc_void_warranty_claim(p_claim_id, p_reason)`](supabase/migrations/20261002000400_warranty_claim_rpcs.sql), [`public.rpc_start_warranty_claim_resolution(p_claim_id)`](supabase/migrations/20261002000500_warranty_claim_resolution_wiring.sql) — **branched by source in Phase 4** ([`20260831002100_consumption_warranty_claim_return_p4.sql`](supabase/migrations/20260831002100_consumption_warranty_claim_return_p4.sql)). Schema + numbering: [`20261002000300_warranty_claims_schema.sql`](supabase/migrations/20261002000300_warranty_claims_schema.sql).
- **Ledger writes:** `warranty_claims` (one row per claim; lifecycle `open → covered/rejected → in_progress → resolved`, plus `void`), `warranty_claim_counters` (per-division numbering). Start-resolution INSERTs a warranty-flagged `so_po_returns` row + its `return_lines`, then flips the claim to `in_progress` with `linked_return_id`. **Sale** claim → `source_type='sale_order'`, `SR-#####`, `return_lines.sale_delivery_line_id`. **Consumption** claim (Phase 4) → `source_type='consumption'`, `CR-#####`, `return_lines.consumption_line_id`, division from the claim (else the consumption entry). Both start `pending_inspection` with a single `condition='inspection'` line.
- **Downstream side-effects:** The created return is resolved entirely through the **existing** Returns machinery (Approach B — no new resolution code). Trigger [`_sync_warranty_claim_from_return`](supabase/migrations/20261002000600_warranty_claim_resolution_type_fix.sql) (AFTER UPDATE on `so_po_returns`, `WHEN warranty_claim_id IS NOT NULL`; **consumption branch added in [`20260831002100`](supabase/migrations/20260831002100_consumption_warranty_claim_return_p4.sql)**) flips the claim to `resolved`. **Sale** → on terminal `resolved_credit` / `resolved_replacement` / `resolved_partial`, with `resolution_type` (replacement / refund / credit; NULL for mixed/partial) + `linked_credit_note_id`. **Consumption** → on terminal `closed` (no customer/credit dimension), `resolution_type` NULL + `linked_credit_note_id` NULL. Invalidations: `warranty.claims`, `warranty.records`, `saleReturns.all` / `consumption-returns`.
- **Dialog / component:** `WarrantyClaimDetailDialog`, `FileWarrantyClaimDialog`, Claims tab in the Warranties page.
- **Guards / preconditions:** All four RPCs are SECURITY DEFINER (REVOKE from PUBLIC/anon; GRANT authenticated). Permission is **source-aware**: a `sale` claim needs `sales.warranty_claims.manage`, a `consumption` claim needs `consumption.warranty_claims.manage` (Start-resolution checks the key by `warranty_type` after loading the claim). Assess requires `status='open'` (reject requires a reason); Start-resolution requires `status='covered'` AND `warranty_type IN ('sale','consumption')` (service/contract raises `0A000` — deferred); Void allowed from any non-terminal status (blocked on `resolved`/`void`, reason required). `rpc_complete_return_inspection` accepts `consumption.returns.*` in addition to `sales/purchase.returns.*` (Phase 4). SELECT is division-scoped RLS; writes are RPC-only. **"Repair" is NOT a customer resolution** — it is an inventory disposition ([[Send Damaged for Repair]]); a *sale* claim resolves only once a customer outcome (refund / replacement / store-credit) is recorded on the linked return, while a *consumption* claim resolves once its return is fully processed back into stock (no customer outcome). Repair-as-a-customer-warranty-outcome belongs to the deferred service/contract workflow.
- **Related flows:** [[Create Warranty Records at Delivery]], [[Create Warranty Records at Consumption]], [[Create Sales Return (Direct)]], [[Consumption Return]], [[Complete Return Inspection]], [[Restock Sales Return]], [[Issue Credit Note for Sales Return]], [[Create Partial Replacement (customer + inventory atomic)]], [[Record Return Refund]], [[Record Return Store Credit]], [[Send Damaged for Repair]]
- **Docs / plans:** [docs/plans/2026-08-21-warranty-completion/03-claims.md](plans/2026-08-21-warranty-completion/03-claims.md), [docs/plans/2026-08-29-consumption-sales-returns-warranty-design.md](plans/2026-08-29-consumption-sales-returns-warranty-design.md) §4 (Phase 4)
- **Notes:** Approach B — `warranty_claims` consumes the Returns machinery, it does not re-implement it. `warranty_type` is snapshotted onto the claim; `sale` and `consumption` are built, service/contract deferred. Claim numbering mirrors `next_warranty_number` (per-division counter). The resolution-sync trigger reads `NEW.credit_note_id` at the sale terminal-status transition — safe because both `_maybe_close_return` and `rpc_close_return` write the terminal status WITHOUT touching `credit_note_id` (it is set earlier at credit-note creation). Phase 4 kept the sale branch of `rpc_start_warranty_claim_resolution` / `rpc_complete_return_inspection` / `_sync_warranty_claim_from_return` verbatim and added the consumption branch beside it; the shared dialog's **"Open Returns" link is source-aware** (`consumption` → `/consumption/returns`). Verified end-to-end on staging via rolled-back probe **and a live UI smoke** (WC-AFM-00003 → CR-00002 → inspect good/damaged → restock → write-off → `closed` → claim `resolved`, consumption COGS net 0).

---

### Consumption Return

- **Module:** Operations / Consumption
- **Status:** Active
- **Trigger surface(s):** [`/consumption/returns`](src/app/(dashboard)/consumption/returns/page.tsx) — **New return** (pick a posted consumption + per-line good/damaged qty; Phase 3b) plus per-row actions **Inspect** (Phase 4; only for `pending_inspection` returns), **Restock good**, **Disposition**, **Cancel**. Also arrives here from [[Warranty Claim (sale + consumption)]] Start-resolution (a covered consumption warranty claim → a `pending_inspection` CR return).
- **Primary hook(s):** [`useConsumptionReturns` / `useReturnableConsumptions` / `useConsumptionReturnableLines` / `useCreateConsumptionReturn` / `useCompleteConsumptionReturnInspection` / `useProcessConsumptionReturnRestock` / `useRecordConsumptionReturnDisposition` / `useCancelConsumptionReturn`](src/hooks/useConsumptionReturns.ts).
- **RPC(s):** [`public.rpc_complete_return_inspection(p_return_id, p_splits, p_restock_warehouse_id)`](supabase/migrations/20260831002100_consumption_warranty_claim_return_p4.sql) (source-agnostic; carries `consumption_line_id` onto the split lines + accepts `consumption.returns.*`), [`public.rpc_process_consumption_return_restock(p_return_id)`](supabase/migrations/20260831002000_consumption_returns_p3b.sql), `public.rpc_record_inventory_disposition(p_return_id, p_warehouse_id, p_dispositions)` (shared). Create + cancel are direct table writes (RLS-guarded). COGS reversal helper: [`public._reverse_sale_cogs_for_return`](supabase/migrations/20260831002000_consumption_returns_p3b.sql) (consumption branch).
- **Ledger writes:** `so_po_returns` (`source_type='consumption'`, `CR-#####`, division from the consumption) + `return_lines` (`consumption_line_id` provenance; enforced by `_enforce_return_line_provenance`). Inspection splits an `inspection` line into `good` / `damaged` lines. Restock good → negative `cogs_entries('consumption_return')` (reverses the original `cogs_entries('consumption')` keyed by `consumption_id`+`brand_variant_id`) + a `consumption_return` `fifo_cost_layers` row + `inventory_stock_movements`. Damaged disposition (write_off / restock_as_damaged) → reverses COGS + routes to Damaged Stock.
- **Downstream side-effects:** `_maybe_close_return` (consumption branch) sets `status='closed'` once good is restocked AND the damaged dispositions are complete. When the return carries a `warranty_claim_id`, closing it flips the claim to `resolved` — see [[Warranty Claim (sale + consumption)]]. Invalidations: `consumption-returns`.
- **Dialog / component:** `/consumption/returns` — inline `CreateConsumptionReturnDialog`, `CompleteInspectionDialog` (Phase 4), `DispositionDialog`, `ConsumptionReturnDetailDialog`.
- **Guards / preconditions:** Returnable qty is `consumed − already_returned` (cancelled returns excluded). Restock requires a `restock_warehouse_id` (no delivery origin to infer) + no pending inspection lines, and is idempotent on `restocked_at`. Permission: `consumption.returns.create` / `consumption.returns.manage`. RLS division-scoped.
- **Related flows:** [[Warranty Claim (sale + consumption)]], [[Create Warranty Records at Consumption]], [[Post Consumption with Milestone tag]], [[Complete Return Inspection]] (sales sibling), [[Send Damaged for Repair]]
- **Docs / plans:** [docs/plans/2026-08-29-consumption-sales-returns-warranty-design.md](plans/2026-08-29-consumption-sales-returns-warranty-design.md) §5.4 (Phase 3b) + §4 (Phase 4)
- **Notes:** Phase 3b built create → restock/disposition → close for operator-initiated returns (born already split good/damaged, status `pending`). Phase 4 added the inspection step so a warranty-resolution CR (born `pending_inspection` with one inspection line) can be split then flow through the same restock/disposition/close path. `rpc_complete_return_inspection` is shared with sales returns — Phase 4 patched it to carry `consumption_line_id` (else the provenance trigger rejected the split lines).
