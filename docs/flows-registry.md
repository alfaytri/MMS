...
- **Status:** Active (DB shipped Phase 9.4 · 2026-07-30; UI in Phase 9.6)
- **Trigger surface(s):** Follow-up dialog opened after a `send_for_repair` disposition is recorded via [[Record Inventory Disposition]]. UI collects vendor + expected return date + notes (Phase 9.6 `SendForRepairDialog`).
...
- **Migrations:** `20260802000500_rpc_send_for_repair.sql`.
...
- **Module:** Warehouse / Sales
- **Status:** Active (DB shipped Phase 9.5 · 2026-07-30; UI in Phase 9.7)
- **Trigger surface(s):** "Return from repair" action on rows in the Out-for-Repair tab of `/warehouse/damaged-stock` (Phase 9.7). Opens `ReturnFromRepairDialog` for outcome + qty inputs.
- **Primary hook(s):** `useReturnDamagedFromRepair` (Phase 9.7).
...
- **Migrations:** `20260802000600_rpc_return_from_repair.sql`.
