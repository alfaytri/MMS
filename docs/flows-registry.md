...
- **Guards / preconditions:** caller must hold `inventory.catalog.manage` (checked server-side via `_user_has_permission(_current_user_data_id(), …)`; both an authenticated non-manager and an unauthenticated caller are rejected with `not authorized`). `p_to_division_id` required (FK-enforced against `company_divisions`); the RPC does not itself forbid transferring to the unit's current division (UI prevents it by excluding that option from the picker). **Table-level backstop (2026-08-27, Task 2b.6):** `trg_guard_tool_unit_division_write` (BEFORE UPDATE on `tool_asset_units`) independently re-checks `inventory.catalog.manage` whenever `division_id` changes, regardless of write path — closes the direct-Edit bypass noted below. Other column edits (condition/status/assigned_to/serial/brand/expiry) stay ungated.
...
- **Notes:** **Gap CLOSED 2026-08-27 (Task 2b.6):** `tool_asset_units` RLS is still `USING (true)`/`WITH CHECK (true)` for ALL commands to `{authenticated}` (unchanged — tightening it would affect the whole unit CRUD path, not just transfer) — any authenticated user can still reach `division_id` via the plain-table path the Edit-Unit dialog uses (`useUpdateToolAssetUnit`). But `trg_guard_tool_unit_division_write` (see Guards, above) now re-checks the same `inventory.catalog.manage` permission at the table level whenever `division_id` changes, so the direct-Edit path can no longer bypass the RPC's gate — both paths now require the same permission. Rolled-back probe (2026-08-27): manager division-change allowed, non-manager division-change blocked (`not authorized to change the owning division of a tool unit`), non-manager non-division edit allowed.
