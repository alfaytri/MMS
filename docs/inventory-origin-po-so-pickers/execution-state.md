...
- **Phase 2 (SO/quotation) — COMPLETE + operator-confirmed 2026-08-09.** All slices shipped on `feature/inventory-origin-po-so-pickers`:
  - **1a — delivery-note PDF origin** (`fetchOriginsByBrandVariant` + `Origin: <country>` per line) — commit `5c2acc63`. ✅ confirmed.
  - **3 — Brand·Origin on SO detail + delivery lines** (`useSaleOrder` joins `brands(name)`+`country_codes(name)`; `variantPickerLabel` in `SoDetailDialog`) — commit `f34bedb2`. ✅ confirmed.
  - **1b — invoice PDF origin** — migration `20260819070000_invoice_line_origin.sql` (+ staging mirror): nullable `invoice_line_items.brand_variant_id` FK + index, both builders (`generate_invoice_from_so`, `rpc_sync_invoice_from_so`) populate it, 8/8 historical lines backfilled (metadata-only, no money change); PDF renders origin via the shared helper — commit `4e61343a`. ✅ confirmed (INV-00003 `Origin: Egypt`).
  - **SO picker → Brand→Origin cascade** — `SoLineItemsEditor` now passes `brandOriginCascade` (this REVERSED the earlier "keep combined list" decision — the user chose the cascade "exactly like the PO", no avail in the picker; stock shows in the breadcrumb after pick) — commit `7b751ab5`. ✅ confirmed.
  - **Item 2 — quotation locks origin+price = NO CODE.** Flow-mapped (two Explore agents): `unit_price` + `brand_variant_id` are frozen onto `sale_order_lines` at quote-time and never re-read from the catalog (edit RPC only recomputes totals; confirm is status-only; the quotation PDF reads stored line values). A later catalog price edit cannot move a quoted number → already satisfied.
  - **Quotation PDF origin — DONE** (commit `3dd525c8`): `Origin: <country>` per line via `hydrateLines`+`fetchOriginsByBrandVariant`. The smoke also exposed + fixed **3 pre-existing bugs** that stopped the quotation PDF generating (schema-migration fallout): `customers.phone` → `customer_phones(phone, is_primary)`, and a phantom `sale_order_lines.item_name_ar` in the select (dropped — Arabic hydrated from the brand-variant). Same `customers.phone` bug fixed in the warranty-certificate generator (`7b9698f2`). Operator-confirmed (SO-2026-08-005 → `Origin: Egypt`).
  - **Remaining:** ⏸ merge/keep decision only (branch NOT pushed). Optional follow-ups left: flip `ReplacementDeliveryDialog` to the cascade (kept combined — ships stock immediately, avail matters); remove dead `useAllBrandNames` (chip).
  - Design: [`phase-2-so-design.md`](phase-2-so-design.md).
