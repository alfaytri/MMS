...
- **Phase 2 (SO/quotation) — COMPLETE + operator-confirmed 2026-08-09.** All slices shipped on `feature/inventory-origin-po-so-pickers`:
  ...
  - **Remaining:** ⏸ merge/keep decision only (branch NOT pushed). Optional follow-ups (not done): origin column on the **quotation PDF** (`generate-quotation-pdf.ts` currently shows no origin column); flip `ReplacementDeliveryDialog` to the cascade (kept combined — ships stock immediately, avail matters).
