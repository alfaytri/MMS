...
### ISSUE-10 — Surface a team's assigned tools on the Custody page 🟡
**Status:** OPEN — **deferred follow-up** (operator chose "existing Custody page, later" at Phase-1 smoke, 2026-08-18).
**Discovered:** 2026-08-18 (operator smoke feedback).
**Description:** The operator wants each team card on `/warehouse/custody` (`CustodyCard` in `src/app/(dashboard)/warehouse/custody/page.tsx`) to also show the **tools that team holds** (from `tool_unit_assignments` / the hub), as a collapsible section like the existing consumable **"Show items" / "Hide items"** expander. The custody sub-container `id` IS the tool-hub `team_id`, so `get_team_tool_units(sub.id)` already returns exactly this.
**Plan when picked up:** add a "Show tools (N)" collapsible beside "Show items". Get the per-team count cheaply up front (one batched read — either `get_teams_with_tool_counts` mapped by `team_id`, or a small count read) and **lazy-load the unit list on expand** (`useTeamToolUnits(sub.id)` with `enabled: expanded`) to respect the Supabase budget rule (no N eager queries). Read-only — no new writes, no permission change (reuses the existing DEFINER read RPC). Do this AFTER the Phase-1 refinements ship, as its own change on `/warehouse/custody`.
**Not built in the Phase-1 refinements batch** (`refinements-plan.md`) — that batch is footer fix + assign tree + Teams-tab division grouping only.
