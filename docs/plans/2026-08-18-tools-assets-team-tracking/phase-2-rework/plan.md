...
- [ ] **Step 1 — RepairTab (awaiting-vendor):** each bucket unit's action becomes **Send for repair** → opens `SendToolForRepairDialog` (vendor Select from `useRepairVendors` — human-readable names; expected-return **calendar** DatePicker, not a text list; optional notes) → `useSendToolForRepair`. The old Repaired/Scrap-from-bucket buttons are removed (superseded by the vendor round-trip).
