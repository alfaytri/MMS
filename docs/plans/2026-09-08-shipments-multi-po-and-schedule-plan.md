...
- [ ] **Step 1: Build the dialog** per the mockup surface ②:
  ...
  - **Multi-PO picker — a searchable combobox, NOT a `<select>`** (reuse the app's cmdk `Command` popover pattern from `CascadeInventorySelector.tsx`; hundreds of POs make a plain dropdown unusable). The picker: (a) is **scoped** server-side to shippable POs — `status IN ('approved','partially_received')` AND having at least one line with un-shipped remainder > 0; (b) **type-ahead** filters on `po_number` + `supplier_name`, capped (`MAX_OPTIONS ≈ 50`) with a "keep typing to narrow" footer; (c) an optional **supplier filter** (a `Select`/chip) narrows to one supplier's open POs, clearable; (d) each result row shows `po_number` · supplier · date · "N open items"; (e) an already-added PO is excluded from results. Provide a `useShippablePurchaseOrders({ search, supplierId })` hook (bounded `.limit(50)`, explicit columns) rather than loading every PO. Adding a PO fetches its `po_line_items` and appends a PO block. Each line row: item name + sku/division, a qty `Input` (default = `remainingUnshipped(po_qty, shippedElsewhere)`; `shippedElsewhere` = the sum from `useLineShippedElsewhere(poLineIds)`), and an "of {po_qty}" hint. A **Whole PO / Clear** toggle sets every line to its remainder / 0. Lines with `exceedsRemaining(...)` show the amber warning row (soft — never blocks). Lines with qty 0 are excluded from submit.
  - **Alternate entry point (add in this task if cheap, else follow-up):** a PO detail action "Add to shipment" (new or existing shipment) prefilled with that PO — so the common single-PO case skips the search entirely.
