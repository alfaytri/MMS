...
| 1.19 | `WhAdjustmentsTab` inline Approve/Reject bypass | Removed the bypass entirely. Every SA now goes through the multi-step chain in `WhAdjustmentDetailDialog`; `canApprove` helper + `reject` inline mutation + inline Approve/Reject buttons deleted; unused `useApproveStockAdjustment` hook removed (underlying RPC still called internally by `action_stock_adjustment_step` and `force_approve_stock_adjustment`). Legacy chain-less SAs deliberately visible-but-unactionable — flag for chain-backfill migration if any surface (`fcff50ba`) | ✅ |
