...
| 1.4 | `bills` | Dropped 5 dead columns: `bill_type`, `source`, `source_id`, `status`, `manually_paid` (`cb7b8bb2`). `bill_recompute_paid_fn` + `allocate_payment_to_bill` rewritten without manual-override branch (same pass fixed a stale enum-cast in the latter). Kept: `source_label` (supplier ref), `receival_id` (3-way match), `division_id`, `doc_status`, `payment_status`, `paid_amount`. `doc_status` + `payment_status` logged as Pass 3 enum candidates | ✅ |
...
| 1.10 | `fifo_cost_layers` | If the layer came from a sale return, it should link back (e.g. `receival_id` or `return_id`). Right now it's not obvious where it originated | ☐ |
