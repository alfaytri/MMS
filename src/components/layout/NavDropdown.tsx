...
const ICON_MAP: Record<string, LucideIcon> = {
  ...
  FileCheck, Clock, Truck, UserCheck, ClipboardList, ClipboardCheck, CheckCircle, Ship,
...
export function NavDropdown({ entry }: NavDropdownProps) {
  ...
  const filteredGroups = useMemo(() => {
    ...
    const admin = permData?.isSystemAdmin ?? false
    return entry.groups
      .map((group) => ({
        ...group,
        items: group.items.filter((item) =>
          canAccess(item.permission, perms, admin)
        ),
      }))
