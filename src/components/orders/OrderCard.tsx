...
export function OrderCard({ order, onClick }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full min-h-11 rounded-lg border border-border bg-white p-3 text-left transition-colors hover:border-border hover:bg-muted space-y-2"
    >
      {/* Row 1: order ID + status + confirmation */}
      <div className="flex items-center gap-2 flex-wrap">
        <span className="font-mono font-semibold text-foreground text-sm">{order.order_id}</span>
        <span className={cn('rounded border px-1.5 py-0.5 text-[11px] font-semibold uppercase', STATUS_STYLES[order.status as OrderStatus] ?? 'bg-muted text-muted-foreground')}>
          {order.status}
        </span>
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
        ...
        {order.scheduled_date && (
          <span className="text-muted-foreground">{format(new Date(order.scheduled_date), 'dd MMM yyyy')}</span>
        )}
        {timeLabel && (
