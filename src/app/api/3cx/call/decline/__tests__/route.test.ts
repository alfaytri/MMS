...
function req(body: unknown): Request {
  return new Request('http://localhost/api/3cx/call/decline', {
    method:  'POST',
...
describe('POST /api/3cx/call/decline', () => {
  ...
  it('400 when callId is missing or not a number', async () => {
    ...
    const res = await POST(req({ callId: 'forty' }))
  ...
  it('200 when drop succeeds — and dropCall was called with the agent\'s ext + participantId', async () => {
    ...
    vi.spyOn(activeCalls, 'fetchActiveCalls').mockResolvedValue([
      {
        ...
        participants:  [
          { extension: '101', participantId: 502 },
          { extension: '112', participantId: 508 },
        ],
        startedAt:     '2026-06-13T08:10:00Z',
      },
    ])
    ...
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ ok: true })
    expect(dropMock).toHaveBeenCalledWith('112', 508)
  })
})
