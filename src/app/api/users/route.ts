...
export async function GET() {
  try {
    ...
    const { data, error } = await (admin as any)
      .from('profiles')
      .select('*, user_custom_roles!user_custom_roles_profile_id_fkey(role_id, custom_roles(name, color)), user_divisions!user_divisions_profile_id_fkey(division_id, divisions(name, short_name, color))')
      .order('full_name')
