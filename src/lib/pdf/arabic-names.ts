 ...
 *
 * fetchOriginsByBrandVariant(client, ids) — a sibling lookup for the origin
 * (country) of a brand-variant line, for PDFs that surface origin per line
 * (delivery note). Traverses brand_variant → country_codes → name. Returns a
 * Map<brand_variant_id, countryName>; missing/null origins are simply absent.
 */
...
export async function fetchOriginsByBrandVariant(
  client: SupabaseClient,
  brandVariantIds: Array<string | null | undefined>,
): Promise<Map<string, string>> {
  const ids = Array.from(new Set(brandVariantIds.filter((x): x is string => !!x)))
  ...
  const { data, error } = await client
    .from('inventory_item_brand_variants')
    .select('id, country_codes(name)')
    .in('id', ids)
  if (error || !data) return new Map()

  const map = new Map<string, string>()
  for (const row of data as unknown as Array<{ id: string; country_codes: { name: string | null } | null }>) {
    const name = row.country_codes?.name
