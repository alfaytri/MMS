  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [teamsRes, employeesRes, vehiclesRes, schedulesRes] = await Promise.allSettled([
        supabase.from('teams').select('*, divisions:company_divisions(id, slug, name, short_name, company_id, companies(id, name_en))').is('deleted_at', null).order('name_en', { nullsFirst: false }).returns<TeamQueryResult[]>(),
