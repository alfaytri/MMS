-- supabase/migrations/20260506000001_fix_approve_adjustment_damage_writeoff.sql
--
-- 'damage' and 'write_off' reduce stock via FIFO deduction, identical to
-- 'decrease'. They are separate types only for reporting/audit purposes.
-- The original RPC only handled 'increase' and 'decrease', causing
-- RAISE EXCEPTION 'Unknown adjustment_type: damage/write_off'.
...
BEGIN
  ...
  ELSIF v_adj.adjustment_type IN ('decrease', 'damage', 'write_off') THEN
    -- All three deduct stock via FIFO; the type is preserved for audit trail
