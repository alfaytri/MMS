-- Extend get_team_leader_visits with the fields the redesigned TL card needs:
--   * notes                (order / site-visit notes)
--   * location_phone       (arrival phone — "Phone When Reach")
--   * other_teams_names[]  (names of the other teams on the same visit)
--   * has_invoice          (does a tl_invoices row already exist for this visit)
-- Also splits the previously-combined phone: customer_phone is now strictly the
-- service customer's primary phone; the arrival phone lives in location_phone.

DROP FUNCTION IF EXISTS public.get_team_leader_visits(uuid, date);

CREATE FUNCTION public.get_team_leader_visits(p_team_id uuid, p_from_date date DEFAULT CURRENT_DATE)
RETURNS TABLE(
  id                 uuid,
  date               date,
  scheduled_time     text,
