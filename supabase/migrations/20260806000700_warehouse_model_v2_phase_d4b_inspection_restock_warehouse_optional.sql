-- Warehouse Model v2 — Phase D.4.b hotfix 4
-- rpc_complete_return_inspection: give p_restock_warehouse_id a DEFAULT NULL so
-- callers can omit it. The value is now informational only (restock derives
-- warehouse + sub-container per-line from the delivery source in D.4.b).

CREATE OR REPLACE FUNCTION public.rpc_complete_return_inspection(
  p_return_id uuid,
  p_splits jsonb,
  p_restock_warehouse_id uuid DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
