-- Report 2.3 — Cash & Cash Equivalents. Movements of cash-equivalent payments
-- (payment_methods.is_cash_equivalent) with an opening balance + running balance.
-- Receipts (direction='incoming') = Debit, payments (outgoing) = Credit.
--
-- payments has no division_id — it is DERIVED from the linked document:
-- sale_order / purchase_order (source_type + source_id) or invoice / bill
-- (invoice_id / bill_id). Verified 2026-08-11: every payment resolves this way.
-- SECURITY DEFINER + is_division_visible on the derived division. The opening
-- balance nets all cash before p_start (within the visible/filtered set); the
-- running balance carries it down the period, ordered by date then created_at.
...
AS $function$
  ...
  SELECT u.is_opening, u.date, u.payment_method, u.doc_no, u.party, u.debit, u.credit, u.balance, u.division_id, u.division_name
  FROM (
    SELECT true AS is_opening, p_start AS date, 'Opening balance'::text AS payment_method,
           NULL::text AS doc_no, NULL::text AS party, NULL::numeric AS debit, NULL::numeric AS credit,
    ...
    UNION ALL
    SELECT false, s.date, s.payment_method, s.doc_no, s.party, s.debit, s.credit,
