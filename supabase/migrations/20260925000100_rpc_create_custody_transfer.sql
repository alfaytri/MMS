-- rpc_create_custody_transfer — custody location -> custody location hand-out.
--
-- A "push": the SOURCE location's responsible person initiates, the stock leaves
-- the source immediately (FIFO deducted, transfer_out booked, exactly like
-- rpc_dispatch_custody_assign), and the transfer is written straight to
-- 'in_transit'. The DESTINATION location's responsible person then accepts it
-- with the existing rpc_accept_custody_assign (unchanged) — the pending banner,
-- Accept dialog, and shortfall/shrinkage handling are all reused as-is.
--
-- Guardrails:
--   * source must be an ACTIVE custody location whose warehouse is flagged
--     can_transfer_custody = true (per-warehouse capability). Teams stay false.
--   * destination must be a different ACTIVE custody location WITH a responsible
--     person (otherwise nobody could accept it).
--   * only the source location's responsible person (or a custody admin) may run it.
--
-- transfer_kind is reused as 'custody_assign' so the transfer flows through the
-- same in_transit -> accept machinery. A custody->custody move is still fully
-- distinguishable in the data: BOTH from_sub_container and to_sub_container sit
-- under custody warehouses (a warehouse->team assign has a non-custody source).
...
-- SECURITY DEFINER in public is callable by PUBLIC by default — lock it to
-- authenticated app users (RLS is bypassed inside; the body does its own auth).
