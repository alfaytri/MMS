-- =============================================================================
-- Warehouse Model v2 — Phase D.2 hotfix: handle PO with NULL division_id.
--
-- Surfaced during D.2 smoke test on a legacy PO whose division was never set.
-- Two bugs in the D.2 receival RPC:
--
--   1. When p_sub_container_id was NOT NULL, the division-match check
--      `v_check_div IS DISTINCT FROM v_division_id` fails when v_division_id
--      is NULL (NULL IS DISTINCT FROM anything = TRUE). Result: even a valid
--      operator-picked sub-container is rejected. Guard the check with
--      `v_division_id IS NOT NULL`.
--
--   2. When p_sub_container_id was NULL AND v_division_id is NULL, the
--      fallback `_find_or_create_sub_container(warehouse, NULL)` tries to
--      auto-create a sub-container with NULL division on a real warehouse,
--      which the Phase C.1 trigger `_enforce_sub_container_division_rule`
--      blocks. Raise a clear operator-facing error instead: the caller must
--      pass p_sub_container_id explicitly (Phase D.2 UI now offers all
--      active sub-containers in that warehouse when PO division is null).
--
-- Everything else preserved verbatim from the 20260803001500 body.
-- =============================================================================
