-- Phase 3a Task 2: write-off return reverses sale COGS + emits the P&L-read
-- damaged_write_off scrap movement. ALSO fixes a pre-existing drift bug: the
-- sale_return_damaged stock movement omitted the now-NOT-NULL sub_container_id.
...
begin
  ...
  for v_disp in select * from jsonb_array_elements(p_dispositions) loop
    ...
    if v_disp_type = 'write_off' then
      ...
      select r.division_id into v_return_div from public.so_po_returns r where r.id = p_return_id;
