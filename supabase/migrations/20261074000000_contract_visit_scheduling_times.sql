-- Team Scheduling — Phase 1 (DB): real from→to times on contract visits + calendar.
--
-- Today the calendar_visits view unions contract visits but HARDCODES their
-- start_time/end_time to NULL (orders/site-visits derive times from a
-- time_slot + duration), so a scheduled contract visit can't sit on the
-- /calendar time axis. This gives contract_visits real start_time/end_time
-- (a from→to block per service; the per-service default duration pre-fills the
-- block, the operator sets the real range) and points the view's contract
-- branch at them. The view is recreated verbatim from its live definition
-- EXCEPT that one branch, and keeps security_invoker=true (do NOT reopen the
-- anon hole the 2026-08-26 prod audit closed).
BEGIN;
...
COMMENT ON COLUMN public.contract_visits.start_time IS
  'Scheduled from-time of this contract visit for its team on scheduled_date; NULL until placed on the schedule.';
COMMENT ON COLUMN public.contract_visits.end_time IS
  'Scheduled to-time; defaults to start + the service default duration when placed, operator adjusts the range.';

CREATE OR REPLACE VIEW public.calendar_visits WITH (security_invoker = true) AS
 SELECT ota.id,
