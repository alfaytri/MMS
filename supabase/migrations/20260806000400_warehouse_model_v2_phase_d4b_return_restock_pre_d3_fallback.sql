-- Warehouse Model v2 — Phase D.4.b hotfix
-- rpc_process_return_restock: pre-D.3 deliveries have cogs_entries.source_id = NULL,
-- so the exact-layer derive path fails for legacy returns. Fall back to
-- (delivery's warehouse_id, return's division_id) via _find_or_create_sub_container
-- — same pattern D.3 cancel_delivery_inventory uses. This is scope-loose but
-- still per-delivery (not warehouse-wide), and is the tightest resolution we
-- can achieve for data that predates D.3.
...
BEGIN
  ...
  LOOP
    ...
    -- Primary: exact-layer derive via cogs_entries.source_id → fifo_cost_layers.
    ...
    -- Fallback for pre-D.3 deliveries (cogs_entries.source_id was added in D.3):
    -- get the warehouse from the delivery and derive a division-scoped
    -- sub-container. Still per-delivery, not warehouse-wide.
    IF v_line_warehouse IS NULL OR v_line_sub_container IS NULL THEN
      ...
      IF v_return.division_id IS NULL THEN
        RAISE EXCEPTION 'Return line %: pre-D.3 delivery has no source_id chain AND return has no division_id; cannot resolve sub-container.',
          v_line.id
          USING HINT = 'Set division_id on the return header or ask ops to reconcile.';
      END IF;

      v_line_sub_container := public._find_or_create_sub_container(v_line_warehouse, v_return.division_id);
    END IF;
