-- Warehouse Model v2 — Phase E follow-up #2
--
-- Symptom: during Phase E smoke, rpc_send_damaged_for_repair raised P0001
-- because the parent so_po_returns row has no division_id. Phase E only
-- checked so_po_returns.division_id and gave up if null.
--
-- Fix: mirror the restock fallback (migration 20260813000100). Cascade is
-- now (so_po_returns.division_id → sale_orders.division_id →
-- cogs_entries.division_id). cogs_entries was preserved by Phase E and is
-- stamped on every delivery COGS row, so it's the safest last-resort
-- ground truth.
--
-- Bodies below are byte-for-byte preserved from the Phase E rewrite
-- (20260810000100_phase_e_rpc_rewrites.sql) — only the division-derive
-- block near the top of each RPC is changed.
...
begin
  ...
  -- Phase E + follow-up: derive source division via cascade
  --   so_po_returns.division_id → sale_orders.division_id → cogs_entries.division_id
  ...
  if v_source_division is null then
    select ce.division_id
      into v_source_division
      from public.so_po_returns r
      join public.cogs_entries   ce on ce.sale_order_id = r.source_id
  ...
  if v_source_division is null then
    raise exception 'rpc_send_damaged_for_repair: cannot derive source division from return, sale_order, or cogs_entries for warehouse %.', p_warehouse_id
      USING HINT = 'Set division_id on the return, sale_order, or delivery COGS row before dispatching to repair.';
  end if;
...
begin
  ...
  -- Phase E + follow-up: derive destination division via cascade
  --   so_po_returns.division_id → sale_orders.division_id → cogs_entries.division_id
  select r.division_id
  ...
  if v_dest_division is null then
    raise exception 'rpc_return_damaged_from_repair: cannot derive destination division from return, sale_order, or cogs_entries.'
      USING HINT = 'Set division_id on the source return, sale_order, or delivery COGS row before receiving the repair transfer.';
  end if;
...
'Warehouse Model v2 Phase E + follow-up. Sends damaged stock to repair vendor.
Division cascade: so_po_returns.division_id → sale_orders.division_id → cogs_entries.division_id.';

COMMENT ON FUNCTION public.rpc_return_damaged_from_repair(uuid, text, numeric, numeric, numeric, text) IS
'Warehouse Model v2 Phase E + follow-up. Receives repair transfer back with good/writeoff split.
Division cascade: so_po_returns.division_id → sale_orders.division_id → cogs_entries.division_id.';
