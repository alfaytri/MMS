-- Fix orders.division: change from the rigid division enum to plain text.
-- The divisions table (slug column) is the source of truth; the enum cast
-- was causing every order creation to fail with a 400 because real division
-- slugs ("Alfaytri", "RSH-Kitchen", etc.) are not valid enum members.
-- Migration 20260501000005 already did the same fix for services.division.

ALTER TABLE orders
  ALTER COLUMN division TYPE text USING division::text;

-- Re-create the RPC without the ::division cast so it accepts any slug.
