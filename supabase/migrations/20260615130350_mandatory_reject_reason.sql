...
-- 1. Stock Adjustment — guard inside the RPC. We CREATE OR REPLACE the
--    whole function since CREATE OR REPLACE is the safe way to update
--    Postgres functions in-place. The body is copied from the latest
--    version (stock_adj_gate_unified migration) with the guard added.
...
-- 2. PO approvals — CHECK constraint as a DB backstop.
ALTER TABLE po_approvals
  DROP CONSTRAINT IF EXISTS po_approvals_rejected_needs_comment_chk;

ALTER TABLE po_approvals
  ADD CONSTRAINT po_approvals_rejected_needs_comment_chk
  CHECK (status <> 'rejected' OR COALESCE(TRIM(comment), '') <> '');

-- 3. Inventory Check approvals — same pattern.
ALTER TABLE inventory_check_approvals
  DROP CONSTRAINT IF EXISTS inv_check_approvals_rejected_needs_notes_chk;

ALTER TABLE inventory_check_approvals
  ADD CONSTRAINT inv_check_approvals_rejected_needs_notes_chk
  CHECK (status <> 'rejected' OR COALESCE(TRIM(notes), '') <> '');
