-- Add missing p_division parameter to create_order_with_dates.
-- The orders table has division NOT NULL, so every insert was failing with a
-- constraint violation (400) because the original RPC omitted this column.

CREATE OR REPLACE FUNCTION create_order_with_dates(
  p_order_id       text,
  p_customer_id    uuid,
  p_type           text,
  p_division       text,    -- division enum slug e.g. "maintenance", "cleaning"
  p_status         text,
