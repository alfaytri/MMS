-- supabase/migrations/20260428000002_rpc_create_sale_order_soft_credit_block.sql
-- Remove the hard credit_exceeded exception for 'confirm' intent.
-- Both 'confirm' and 'quotation' now save as pending_approval when credit is exceeded,
-- instead of blocking the confirm entirely. Owner approval flow handles the rest.
BEGIN;
...
BEGIN
  ...
  -- Both intents: if credit is exceeded → pending_approval (owner must approve).
  -- 'confirm' within credit → confirmed immediately.
  -- 'quotation' within credit → quotation.
  v_so_status := CASE
