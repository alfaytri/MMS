-- supabase/migrations/20260426000002_fix_apply_receival_edit_po_sync.sql
--
-- BUG: apply_receival_edit updates FIFO layers and inventory stock when a receival
-- quantity is edited, but never syncs the delta back to po_line_items.received_qty.
-- This means the PO shows the wrong received count after an edit (e.g. still "Fully
-- Received" after reducing qty from 50 to 45), blocking future receivals.
--
-- FIX: fetch po_line_item_id from receival_items in the existing SELECT, then apply
-- the qty delta to po_line_items.received_qty before the inventory CONTINUE guard.
-- This mirrors the symmetry already in approve_receival_inventory (rejection path).
...
DECLARE
  ...
  v_ri              RECORD;
  v_bv_id           UUID;
  v_pli_id          UUID;   -- NEW: po_line_item_id for PO sync
  v_old_qty         INT;
  v_new_qty         INT;
  v_old_cost        NUMERIC;
  v_new_cost        NUMERIC;
...
BEGIN
  ...
  FOR v_item_input IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Fetch receival item — include po_line_item_id for PO sync
    ...
    -- Sync PO line item received_qty regardless of inventory linkage.
    -- This mirrors the pattern in create_and_approve_receival and the rejection
    -- rollback in approve_receival_inventory.
    ...
    -- Skip inventory (FIFO / stock_level) updates for items with no variant link
