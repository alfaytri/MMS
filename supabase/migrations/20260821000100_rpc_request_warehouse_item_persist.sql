-- Persist the "item needed" request as a warehouse_item_requests row AND drop an
-- actionable notification per warehouse RP pointing at that row. Return type
-- changes int -> uuid (the new request id), so DROP + CREATE + re-GRANT.
-- A warehouse with no RP is no longer fatal: the request is still recorded and
-- shows on the Requested Items tab for super-viewers/admins.
...
begin
  ...
  -- One actionable notification per responsible person, pointed at the request row.
  for v_rp in
