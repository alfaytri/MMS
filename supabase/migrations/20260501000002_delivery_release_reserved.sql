-- Fix complete_delivery_inventory to also release reserved_qty.
-- Previously the RPC deducted stock_level (via FIFO) but never cleared the
-- reservation, leaving reserved_qty permanently inflated after delivery.
...
BEGIN
  ...
  LOOP
    ...
    -- Deduct FIFO layers (updates stock_level) — raises if insufficient stock
    SELECT total_cost, weighted_unit_cost
    INTO v_result
    FROM deduct_fifo_layers(v_bv_id, v_wh_id, v_qty, false);

    -- Release reservation — clamp at 0 to guard against double-releases
