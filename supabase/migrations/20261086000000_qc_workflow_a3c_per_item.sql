-- QC A3c — per-item scoring: surface each service's qc_items checklist to the
-- team-leader QC visit so scoring is per checklist item (out of its max_score)
-- instead of one flat 0–10 per service.
--
-- Only change vs the LIVE get_team_leader_visits: Source 1 (order visits) now
-- includes 'qc_items' (the service's {label,max_score}[]) in each service_json
-- entry. The loader flattens these into QcItem scorers; the QC dialog already
-- renders visit.qc_items. Body is otherwise the live definition verbatim.
  ...
  SELECT
    ...
    (
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'qc_items', s.qc_items           -- A3c: per-item QC checklist {label,max_score}[]
      ) ORDER BY os.name)
