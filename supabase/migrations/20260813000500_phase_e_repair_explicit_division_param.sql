-- Warehouse Model v2 — Phase E follow-up #5
--
-- Symptom: send-for-repair still fails when
--   - the SO + all its cogs_entries + the return have no division_id
--     (legacy pre-division data), AND
--   - the source warehouse has >1 sub-container (so no unambiguous derive).
--
-- Fix: add an explicit p_source_division_id parameter. The UI passes the
-- operator's active division. When present it wins over the entire derive
-- cascade — this is the operator's authoritative choice for where the
-- stock lives.
--
-- Only send-for-repair takes the new param; return-from-repair still
-- derives via the cascade because the destination sub-container is
-- constrained by where the stock originally left from (v_transfer already
-- carries that context).
--
-- Body preserved from 20260813000400; only signature + first cascade
-- step changed. RPC signature adds a nullable parameter with default, so
-- existing callers still work.

-- Drop the previous overload (5-arg) before adding the 6-arg version to
-- keep pg_proc clean of stale signatures. The 5-arg one was created by
-- 20260810000100 and rewritten by 000200 / 000300 / 000400.
DROP FUNCTION IF EXISTS public.rpc_send_damaged_for_repair(uuid, uuid, uuid, date, text);
...
CREATE OR REPLACE FUNCTION public.rpc_send_damaged_for_repair(
  ...
  p_source_division_id         uuid  DEFAULT NULL
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
COMMENT ON FUNCTION public.rpc_send_damaged_for_repair(uuid, uuid, uuid, date, text, uuid) IS
'Warehouse Model v2 Phase E + follow-up #5. Cascade:
  p_source_division_id (explicit override)
  → return.division_id
  → sale_orders.division_id (SO) / purchase_orders.division_id (PO)
  → cogs_entries.division_id (SO only, any variant on the SO)
  → source warehouse single-sub-container fallback.';
