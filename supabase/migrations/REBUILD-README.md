...
**Status:** `warehouse/` reconstructed from live staging 2026-09-02; the whole-app DB
`wkmvjxxmzstsvahuiwsz` had its warehouse domain repaired to match current staging (2026-09-02),
then `whole-app/` was regenerated byte-exact from that repaired DB. Both count-validated against
their live catalogs, neither test-applied yet (no Docker). **Review before use.**
...
- **`whole-app/`** — the full app (warehouse **plus** the field-service modules: team-leader +
  ...
  `profiles` gone, all renames applied. On 2026-09-02 the DB's **warehouse domain was reconciled to
  current staging** (see `whole-app/_repairs/`) while keeping all module objects, then this baseline
  was regenerated from the repaired DB. 187 public tables = 114 warehouse + 73 module. Public schema
  only; RLS + policies kept, grants and non-`public` objects out of scope. See its README.
