-- Add source_id column to calendar_visits so consumers can open the underlying
-- order / site_visit / follow_up_request without an extra lookup. `id` stays
-- as the assignment row's id (unique per row in the union); `source_id` is the
-- canonical id of the underlying record (orders.id, site_visits.id, etc.).
...
COMMENT ON VIEW public.calendar_visits IS
  'Unified calendar over orders + contract_visits + site_visits + pending follow_up_requests. Read-only. `id` is the row id within the source table; `source_id` is the canonical id of the underlying record (orders.id, site_visits.id, etc.).';
