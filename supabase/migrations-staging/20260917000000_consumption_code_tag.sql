-- Projects consumption — a manual free-text "code" tag under the milestone.
--
-- Operators type a code (a cost / WO / drawing ref) when consuming to a PROJECT
-- pool, so spend groups project → discipline → milestone → CODE → items. Stored
-- on the consumption header (consumption_entries) and copied to the COGS ledger
-- (cogs_entries), mirroring how milestone_id / discipline_id already flow.
--
-- The consumption dialog REQUIRES the code for project spend (a UI gate), the
-- same way discipline + milestone are UI-required today. The column stays
-- nullable and the RPC does NOT hard-require it: a non-project consumer, or an
-- older client that predates this field, still posts (code just stays null).
-- Keeping p_code DEFAULT NULL also makes the new param backward compatible, so
-- the DB and the frontend can deploy in either order without breaking posting.
--
-- Both RPCs are recreated from their LIVE bodies (fetched via pg_get_functiondef)
-- with only the code additions. rpc_post_consumption gains a param → DROP+CREATE
-- (CREATE OR REPLACE can't change the signature); rpc_report_project_consumption
-- gains a return column → DROP+CREATE. Grants (authenticated, service_role; no
-- PUBLIC/anon) are re-applied per the no-anon-execute invariant.
BEGIN;

-- ─────────────────────────────────────────────────────────────────────
-- 1. code column on the consumption header + the COGS ledger
-- ─────────────────────────────────────────────────────────────────────
ALTER TABLE public.consumption_entries ADD COLUMN IF NOT EXISTS code text;
ALTER TABLE public.cogs_entries        ADD COLUMN IF NOT EXISTS code text;
CREATE INDEX IF NOT EXISTS cogs_entries_code_idx
  ON public.cogs_entries (code) WHERE code IS NOT NULL;

-- ─────────────────────────────────────────────────────────────────────
-- 2. rpc_post_consumption — accept p_code, require it for project spend,
--    store it on the header + copy to each cogs row.
-- ─────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.rpc_post_consumption(uuid, uuid, text, uuid, text, text[], jsonb, uuid, uuid);
...
CREATE OR REPLACE FUNCTION public.rpc_post_consumption(
  ...
  p_attachments text[],
  p_lines jsonb,
  p_milestone_id uuid DEFAULT NULL::uuid,
  p_discipline_id uuid DEFAULT NULL::uuid,
...
begin
  ...
  insert into public.consumption_entries (
    ...
    division_id, milestone_id, discipline_id, code
  ) values (
    ...
    v_sub.division_id, p_milestone_id, p_discipline_id, v_code
  )
...
REVOKE ALL ON FUNCTION public.rpc_post_consumption(uuid, uuid, text, uuid, text, text[], jsonb, uuid, uuid, text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.rpc_post_consumption(uuid, uuid, text, uuid, text, text[], jsonb, uuid, uuid, text) TO authenticated, service_role;

-- ─────────────────────────────────────────────────────────────────────
-- 3. rpc_report_project_consumption — surface the code as a grouping level
--    (project → discipline → milestone → CODE → item → date).
-- ─────────────────────────────────────────────────────────────────────
