-- Phase 1: require notes on CUSTODY consumption (the invoice / order / project
-- ref). Custody consumption is operationally a sale, so its reference must be
-- captured. Internal consumption is unaffected.
--
-- Body reproduced verbatim from the live definition (pg_get_functiondef of
-- rpc_post_consumption, last set in 20260919000000_consumption_is_team_item_stamp.sql);
-- the ONLY change is the custody-notes guard added right after the consumer_type
-- validations, before any FIFO drain.
