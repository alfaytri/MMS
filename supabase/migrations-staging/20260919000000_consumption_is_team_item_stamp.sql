-- 20260919000000_consumption_is_team_item_stamp.sql
-- Team vs Service consumption split — stamp consumption_entries.is_team_item.
--
-- rpc_post_consumption derives whether a consumption is of TEAM-held items and
-- stamps the header, so the Consumption page can split its two history lists
-- (Team vs Service) with a cheap, stable, indexed filter instead of a per-row
-- join. Effective flag per item = COALESCE(item.is_team_item, category.is_team_item,
-- false) (column added in 20260918000000).
--
-- A single consumption is HOMOGENEOUS by construction — the UI's Team picker only
-- offers team-items and the Service picker excludes them, so one entry never mixes
-- the two. We assert that here (collect the DISTINCT effective flags and require
-- exactly one) so the single stored value is never ambiguous; a hand-crafted mixed
-- post is rejected rather than silently misfiled.
--
-- Backward compatible + no behaviour change for existing flows: every current item
-- is is_team_item=false (nothing flagged yet), so every consumption stamps false
-- (Service) exactly as today. No new parameter, no signature change → CREATE OR
-- REPLACE (grants preserved; re-applied anyway per the no-anon-execute invariant).
-- Body is otherwise byte-for-byte the live 20260917000000 definition
-- (drift-checked via pg_get_functiondef before rewrite).
BEGIN;
