-- Warehouse Model v2 — Phase D.4 Task 5
-- `approve_stock_adjustment_inventory` passes `v_sub_container_id` as the
-- 5th arg to `deduct_fifo_layers` so the drain on the
-- decrease/damage/write-off branch stays inside the adjustment's own
-- sub-container. The C.2.d rewrite already resolves + stamps
-- `sub_container_id` on the adjustment row, the created FIFO layer
-- (increase branch), and every movement — only the deduct call was still
-- warehouse-wide.
--
-- Live body sourced 2026-08-01 via pg_get_functiondef; the only delta is
-- the 5th arg on the FOR-loop over `deduct_fifo_layers`. Everything else
...
BEGIN
  ...
  ELSIF v_adj.adjustment_type IN ('decrease', 'damage', 'write_off') THEN
    ...
    -- One movement per layer drained. `deduct_fifo_layers` gets the 5th
    -- arg so the drain stays inside the adjustment's sub-container; without
    -- it a decrease/damage/write-off could still span sub-containers of the
    -- same warehouse.
    FOR v_layer IN
