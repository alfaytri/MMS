-- Consumption access control — enforce source assignment in the DB.
--
-- Problem: rpc_post_consumption validated that the source sub-container belongs
-- to the source warehouse + is active, but NOT that the caller is allowed to
-- draw from it. Any authenticated user could post a consumption from ANY
-- warehouse/custody via a direct RPC call — hiding it in the UI is not enough.
--
-- Rule (matches the operator's intent): a user may post a consumption only from
--   (a) a real warehouse they are the responsible person of (is_field_rp_of /
--       warehouse_responsible_persons), OR
--   (b) a custody sub-container they are the responsible person of
--       (warehouse_sub_containers.responsible_person_profile_id) — Teams/Places,
-- with a manager/admin override (_has_custody_admin_role = inventory_manager OR
-- system admin) for people who oversee everything. Not division-wide.
--
-- Only a new guard block is added (right after the source-belongs check); the
-- rest of the body is the live definition (numbering + per-layer COGS) verbatim.
-- Custody REQUEST (rpc_create_custody_assign) is unchanged — operators may still
-- browse the main warehouses to request stock into their custody.
