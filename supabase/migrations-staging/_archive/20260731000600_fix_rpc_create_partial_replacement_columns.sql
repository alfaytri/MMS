-- Phase 8 Sub-task 8.7 sub-fix: rpc_create_partial_replacement was writing to
-- six columns that don't exist on sale_deliveries / sale_delivery_lines.
--
-- The 20260730000500_fix_write_off_double_movement migration re-wrote the RPC
-- and (while fixing the double-movement bug) stubbed generic column names into
-- the INSERT statements without checking the real table shapes. The bug only
-- manifests when a caller actually goes through the replacement-delivery arm
-- (p_lines non-empty OR p_gift_items non-empty). The 7.7 walkthrough Section D
-- exercised only the disposition arm, so this went unnoticed until 8.7.
--
-- Discovered on 2026-07-29 during Phase 8.7 Section D when the ReplacementDeliveryDialog
-- surfaced: column "delivery_date" of relation "sale_deliveries" does not exist.
--
-- Column corrections vs. the current live body (pg_get_functiondef 2026-07-29):
--   sale_deliveries.delivery_date  → date
--   sale_deliveries.division_id    → (drop; column not on this table)
--   sale_deliveries.notes          → (drop; column not on this table)
--   sale_delivery_lines.qty        → qty_delivered
--   sale_delivery_lines.unit_price → (drop; column not on this table)
--   sale_delivery_lines.is_gift    → (drop; column not on this table)
-- Also restored: sale_deliveries.return_id (present in 20260730000300, dropped
-- in 000500) — links replacement delivery back to its source return.
...
comment on function public.rpc_create_partial_replacement(uuid, uuid, jsonb, jsonb, jsonb) is
