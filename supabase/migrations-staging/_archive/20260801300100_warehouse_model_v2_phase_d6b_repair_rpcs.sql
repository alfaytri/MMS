-- Warehouse Model v2 — Phase D.6.b Task 2: Rewrite Phase 9 repair RPCs
--
-- Before D.6.b, each vendor had its own virtual warehouse with a single
-- auto-created sub-container. Both RPCs looked up the vendor's sub-container
-- via `SELECT id FROM warehouse_sub_containers WHERE warehouse_id =
-- v_vendor.virtual_warehouse_id LIMIT 1`.
--
-- After D.6.b, all vendors share ONE Repair warehouse and each is a distinct
-- sub-container under it — that LIMIT 1 lookup would now pick a random
-- vendor's sub. Fix: use `repair_vendors.sub_container_id` directly on the
-- send RPC, and `warehouse_transfers.to_sub_container_id` (already stamped
-- by the send RPC) on the return RPC.
--
-- Live bodies sourced 2026-08-01 via pg_get_functiondef and used as the
-- base — see previous session notes. Only the sub-container lookup lines
-- change; the rest of the FIFO / stock-movement / transfer logic is
-- identical.
...
--    Lookup change: fetch v_vendor.sub_container_id and use it directly
--    for v_to_sub_container_id (was: LIMIT 1 by warehouse_id).
-- ─────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- D.6.b: also fetch sub_container_id — the vendor's distinct sub under
  -- the shared Repair warehouse. Pre-D.6.b callers had one sub per vendor
  -- warehouse; post-D.6.b, this field is NOT NULL and directly addresses it.
  ...
  --   Source (real warehouse): find-or-create against the source division.
  --   Destination (shared Repair warehouse): use the vendor's sub_container_id
  --   directly. Pre-D.6.b did SELECT ... LIMIT 1 — that would pick a random
  --   vendor's sub post-consolidation because all vendors' subs share the
  --   same warehouse_id now.
...
--    Lookup change: use v_transfer.to_sub_container_id (stamped by the
--    send RPC) as v_from_sub_container_id for the return direction.
--    Was: SELECT ... LIMIT 1 by v_wh_vendor.
-- ─────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- D.6.b: also fetch to_sub_container_id — stamped by the send RPC as the
  -- vendor's specific sub-container. Post-consolidation, LIMIT 1 by
  -- warehouse_id would pick a random vendor's sub because all vendors' subs
  -- share the same Repair warehouse.
  ...
  -- Resolve sub-containers for the inbound direction.
  --   From (vendor sub in the shared Repair warehouse): reuse the sub id
  --   stamped by the send RPC on this same transfer's to_sub_container_id.
  --   To (destination real warehouse): find-or-create against its division.
