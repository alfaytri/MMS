-- Warehouse Model v2 — Phase D.4.b hotfix 2
-- rpc_process_return_restock: extend the pre-D.3 fallback chain for division
-- resolution. Some legacy returns predate division propagation and have no
-- division_id on the return header OR the sale_order. Fall through:
--   return.division_id → sale_orders.division_id → warehouses.division_id
-- and only hard-fail if all three are NULL.
...
BEGIN
  ...
  LOOP
    ...
    IF v_line_warehouse IS NULL OR v_line_sub_container IS NULL THEN
      ...
      -- Division-derive cascade: return → SO → warehouse.
      ...
      IF v_fallback_division IS NULL THEN
        ...
        WHERE  w.id = v_line_warehouse;
      END IF;

      IF v_fallback_division IS NULL THEN
        RAISE EXCEPTION 'Return line %: pre-D.3 delivery has no source_id chain AND division cannot be resolved from return, sale_order, or warehouse.',
          v_line.id
          USING HINT = 'Ask ops to set division_id on the warehouse or the return before restocking.';
      END IF;
