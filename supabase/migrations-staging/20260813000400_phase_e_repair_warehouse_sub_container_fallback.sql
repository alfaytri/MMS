-- Warehouse Model v2 — Phase E follow-up #4
--
-- Symptom: send-for-repair still failed after #3. The SO, its return, and
-- ALL of its cogs_entries have no division_id (legacy pre-division SO).
--
-- Fix: broaden the cascade with two more layers before giving up.
--
--   4. cogs_entries.division_id for the SO with brand_variant_id filter DROPPED
--      — any COGS row on this SO with a division counts, not just this variant's.
--   5. Source warehouse's single sub_container division — if p_warehouse_id
--      has exactly one warehouse_sub_containers row, use its division. Safe
--      because it's deterministic: with one sub-container there's no
--      ambiguity about which division owns the stock at this warehouse.
--
-- If both fail we still raise, but this covers pre-division legacy data
-- landing in a single-sub-container warehouse (the common case).
--
-- Bodies preserved from 20260813000300 — only the cascade extended.
...
begin
  ...
  -- Broadened: any cogs_entries row on this SO with a division_id, not just
  -- one matching this brand_variant_id.
  if v_source_division is null and v_return.source_type = 'sale_order' then
  ...
  -- Last resort: source warehouse has exactly one sub_container → use it.
  ...
  if v_source_division is null then
    raise exception 'rpc_send_damaged_for_repair: cannot derive source division. Return %/% has no division_id, parent %/% has no division_id, no cogs_entries division stamped, and warehouse % has % sub-containers.',
      v_return.source_type, v_return_line.return_id,
      v_return.source_type, v_return.source_id,
      p_warehouse_id, v_sub_ct
      USING HINT = 'Set division_id on the return, or on the parent sale_order/purchase_order.';
  end if;
...
begin
  ...
  if v_dest_division is null then
    select count(*)
      into v_sub_ct
      from public.warehouse_sub_containers wsc
      where wsc.warehouse_id = v_wh_source;
  ...
  if v_dest_division is null then
    raise exception 'rpc_return_damaged_from_repair: cannot derive destination division. Return %/% has no division, parent %/% has no division, no cogs_entries stamped, and destination warehouse % has % sub-containers.',
      v_return_row.source_type, v_disp_id,
      v_return_row.source_type, v_return_row.source_id,
      v_wh_source, v_sub_ct;
  end if;
