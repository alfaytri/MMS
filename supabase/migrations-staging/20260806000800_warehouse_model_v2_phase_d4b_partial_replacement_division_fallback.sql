-- Warehouse Model v2 — Phase D.4.b hotfix 5
-- rpc_create_partial_replacement: the write_off fallback path uses
-- return.division_id which is NULL for legacy returns. Add the same cascade
-- fallback used in rpc_process_return_restock: return → sale_order → warehouse.
  ...
  IF jsonb_typeof(p_dispositions) = 'array' AND jsonb_array_length(p_dispositions) > 0 THEN
    FOR v_disp IN SELECT * FROM jsonb_array_elements(p_dispositions) LOOP
      ...
      IF v_disp_type = 'write_off' THEN
        ...
        -- Primary: exact-layer derive via delivery_line → cogs → fifo.
        ...
        -- Fallback: (p_warehouse_id, resolved division). Division cascade:
        -- return → sale_order → warehouse (same as rpc_process_return_restock).
        IF v_disp_warehouse IS NULL OR v_disp_sub_cont IS NULL THEN
          ...
          IF v_fallback_div IS NULL THEN
            SELECT w.division_id INTO v_fallback_div
            FROM   public.warehouses w WHERE w.id = p_warehouse_id;
          END IF;

          IF v_fallback_div IS NULL THEN
            RAISE EXCEPTION 'rpc_create_partial_replacement: write_off cannot resolve division from return, sale_order, or warehouse for warehouse %.',
              p_warehouse_id
              USING HINT = 'Set division_id on the warehouse or return before writing off.';
          END IF;
