-- Fix allocate_warehouse_stock to handle opening/migrated stock that exists in
-- stock_level but has NO fifo_cost_layers records at all (e.g. stock imported
-- from Odoo or entered manually before FIFO tracking was enabled).
--
-- When increasing a warehouse's qty, the RPC now checks for an "opening stock
-- gap" = stock_level minus total remaining across ALL fifo layers. Units in
-- that gap are materialised as FIFO layers dated 2000-01-01 so they are
-- deducted FIRST under FIFO ordering (oldest date wins), satisfying the rule
-- that pre-existing / Odoo-migrated stock sells before new PO receipts.
-- Crucially, stock_level is NOT incremented for these layers because the
...
DECLARE
  ...
  v_stock_level    INT;
  v_opening_gap    INT;
  v_to_reassign    INT;   -- from NULL-warehouse layers
  v_from_gap       INT;   -- from opening stock gap (no layers yet)
  v_to_create      INT;   -- truly new stock (increments stock_level)
  r                RECORD;
  v_remaining      INT;
  v_take           INT;
BEGIN
  ...
  IF v_delta > 0 THEN
    ...
    -- Units that live in stock_level but have no FIFO layer at all yet.
    SELECT COALESCE(SUM(remaining_qty), 0)
    INTO v_total_fifo
    FROM fifo_cost_layers
    WHERE brand_variant_id = p_brand_variant_id
    ...
    IF v_from_gap > 0 THEN
      ...
      -- stock_level is NOT incremented — this stock already exists.
    END IF;
