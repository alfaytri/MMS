-- Add an end-of-function verification to allocate_warehouse_stock.
-- After all qty changes, the function now confirms that the warehouse's
-- actual FIFO remaining_qty equals p_target_qty. If it does not, a
-- RAISE EXCEPTION surfaces the discrepancy so the client sees a real
-- error instead of a silent no-op followed by a false success toast.
--
-- This migration is purely additive (appends the check after the
-- existing PERFORM recalc_average_cost call).
...
BEGIN
  ...
  -- layers for this warehouse so historical zero-cost layers are corrected.
  IF p_unit_cost > 0 THEN
    UPDATE fifo_cost_layers
    SET unit_cost       = p_unit_cost,
        total_unit_cost = p_unit_cost
  ...
  -- ── Verification ────────────────────────────────────────────────────────
  -- Confirm the warehouse now holds exactly the requested qty.
  -- If it doesn't, surface a clear error rather than silently lying to the UI.
  SELECT COALESCE(SUM(remaining_qty), 0)
  INTO v_final_qty
  FROM fifo_cost_layers
  WHERE brand_variant_id = p_brand_variant_id
  ...
  IF v_final_qty <> p_target_qty THEN
    RAISE EXCEPTION
      'allocate_warehouse_stock: qty mismatch after allocation — expected %, got % (delta was %, reassigned %, from_gap %, new %)',
