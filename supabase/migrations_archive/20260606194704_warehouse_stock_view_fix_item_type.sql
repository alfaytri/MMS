-- Fix item_type: leaf categories may not have type set — fall back to parent's type.
-- Also fix coalesce order for category_name: prefer parent name (higher level) as Level-1 label.
