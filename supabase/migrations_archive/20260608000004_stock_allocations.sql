-- Stock allocations table + updated warehouse_stock_view with allocated/available qty
BEGIN;

-- 1. Allocations table (composite PK, no surrogate id)
CREATE TABLE warehouse_stock_allocations (
...
ALTER TABLE warehouse_stock_allocations ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read warehouse_stock_allocations"
  ON warehouse_stock_allocations FOR SELECT TO authenticated USING (true);

-- 2. Recreate warehouse_stock_view with allocated_qty and available_qty
