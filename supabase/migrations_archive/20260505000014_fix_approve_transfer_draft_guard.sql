-- supabase/migrations/20260505000014_fix_approve_transfer_draft_guard.sql
--
-- The NOT IN guard included 'draft', which is not a valid transfer_status enum value.
-- PostgreSQL tries to cast every literal in the IN list to the enum type, so the
-- comparison itself raises: "invalid input value for enum transfer_status: 'draft'".
-- Remove 'draft' from the guard — only 'pending', 'in_transit', and 'pending_approval'
-- are valid approvable statuses.
