-- ─────────────────────────────────────────────────────────────────────────────
-- create_order_with_dates — pre-check pending follow_up_requests
--
-- Pending follow-up requests reserve a team+date+window. Without this check
-- the RPC only rejects on the unique constraint of order_team_assignments,
-- which doesn't fire against follow_up_requests. So a normal-order booking
-- could silently overlap a customer follow-up ask.
--
-- This migration REPLACES the RPC body with the previous version (from
-- 20260520180000_booking_confirmation_setup.sql) plus an explicit overlap
-- pre-check before each insert into order_team_assignments.
-- ─────────────────────────────────────────────────────────────────────────────
