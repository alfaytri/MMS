...
🚀 Staging environment setup — fresh Supabase project (`kfykuifatnmsdcziaybk`) provisioned; Vercel project linked to `staging` branch (user action pending). Migration history squashed into a single baseline file so any fresh DB push reproduces prod exactly.
